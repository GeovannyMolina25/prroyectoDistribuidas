...
const getUserById = async (req, res) => {
  try {
    ...
    if (response.rows && response.rows.length > 0) {
      ...
      res
        .status(404)
        .json({ message: "No se encontraron datos del conductor" });
    }
  } catch (error) {
...
const getUserByName = async (req, res) => {
  try {
    ...
    const response = await pool.query(
      "SELECT * FROM conductor WHERE nombre_con = $1",
      [nombre]
    );
    ...
    if (response.rows && response.rows.length > 0) {
      ...
      res
        .status(404)
        .json({
          message: "No se encontraron conductores con el nombre proporcionado",
        });
    }
  } catch (error) {
...
const getUserByIdCombustible = async (req, res) => {
  ...
  const response = await pool.query(
    "Select * from ControlCombustible  WHERE id_controlcombustible = $1",
    [id]
  );
  res.json(response.rows);
};
...
const updateUser = async (req, res) => {
  ...
  const response = await pool.query(
    "UPDATE Conductor SET nombre_con = $1, apellido_con = $2, cedula_con = $3, numero_Telefono_con = $4, correo_con = $5, fecha_de_nacimiento_con = $6 WHERE id_conductor = $7",
    [
      nombre_con,
      apellido_con,
      cedula_con,
